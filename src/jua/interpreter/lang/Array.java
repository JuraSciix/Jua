...
public class Array implements Cloneable {
    ...
    public static <K, V> Array fromMap(Map<K, V> map, OperandFunction<K> kc, OperandFunction<V> vc) {
        Array array = new Array();
        map.forEach((key, value) -> array.set(kc.apply(key), vc.apply(value)));
        return array;
    }

    public static <E> Array fromCollection(Collection<E> collection, OperandFunction<E> conv) {
        Array array = new Array();
        collection.forEach(e -> array.add(conv.apply(e)));
        return array;
    }

    public static Array fromArray(int[] values) {
        Array array = new Array();
        for (int value: values) array.add(IntOperand.valueOf(value));
        return array;
    }

    public static Array fromArray(long[] values) {
        Array array = new Array();
        for (long value: values) array.add(IntOperand.valueOf(value));
        return array;
    }

    public static Array fromArray(short[] values) {
        Array array = new Array();
        for (short value: values) array.add(IntOperand.valueOf(value));
        return array;
    }

    public static Array fromArray(byte[] values) {
        Array array = new Array();
        for (byte value: values) array.add(IntOperand.valueOf(value));
        return array;
    }
    ...
    public static <T> Array fromArray(T[] values, OperandFunction<T> conv) {
        Array array = new Array();
        for (T value: values) array.add(conv.apply(value));
        return array;
    }
    ...
    public synchronized void add(Operand value) {
        map.put(IntOperand.valueOf(map.size()), value);
    }
    ...
    private Operand checkKey(Operand key) {
        ...
        switch (type) {
            case FLOAT: return IntOperand.valueOf(key.intValue());
            case BOOLEAN:
            case STRING:
            case INT: return key;
